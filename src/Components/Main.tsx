  ...
  return (
    <div id='Main' className='w-full h-screen text-center'>
        <div className="bg-[url('/saturn2.png')] bg-cover bg-no-repeat w-full h-full mx-auto bg-fixed text-center">
            <div>
                <h1 className='text-7xl text-white opacity-75 text-center font family-Georgia tracking-widest pt-80'>
                    SATURN
                </h1>
                <div className='text-3xl text-white text-center pt-44'>
                    {ButtonLinkType.map((item)=>(
                        <div key={item.link}>
                            <ButtonLink link={item.link} title={item.title}/>
                        </div>
                    ))}
                </div>
                <Link href="#Saturn">
